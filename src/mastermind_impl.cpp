...
void mastermind_t::data::reconnect() {
	...
	do {
		...
		try {
			...
			MM_LOG_ERROR(m_logger, "libmastermind: {}: service_error: {}; host: {}", __func__, e.what(), remote);
		} catch (const std::exception &e) {
			MM_LOG_ERROR(m_logger, "libmastermind: {}: {}; host: {}", __func__, e.what(), remote);
		}
	...
	MM_LOG_ERROR(m_logger, "libmastermind: {}: cannot reconnect to any host", __func__);
...
void mastermind_t::data::collect_info_loop() {
	...
	try {
		...
		MM_LOG_ERROR(m_logger, "libmastermind: reconnect: {}", e.what());
	}
